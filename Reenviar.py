    ...
    try:
        ...
        for media_msg in media_group_msgs:
            try:
                ...
                # Para el canal privado, solo se envía el nombre del tipster, sin estadísticas
        ...
        # Enviar todas las imágenes al canal privado con el nombre del tipster
...
def generar_mensaje_con_estadisticas(tipster, datos_tipster):
    ...
    if bank_actual is not None and not math.isnan(bank_actual):
        # Asignar símbolo + si el bank actual es positivo
        simbolo = "+" if bank_actual > 0 else ""
        mensaje += f"💰 Balance: {simbolo}${bank_actual:,.2f}\n"
    ...
    if victorias or derrotas:  # Solo mostrar si hay victorias o derrotas
